...
def loadGs():
    with g_LoadingMutex:
        ...
        model_path = 'models/stylegan2-ffhq-config-f.pkl'
        ...
        with open(model_path, 'rb') as f:
            with g_Session.as_default():
                ...
                g_dLatentsIn = tf.placeholder(
                    tf.float32, [Gs.components.synthesis.input_shape[1] * Gs.input_shape[1]])
                dlatents_expr = tf.reshape(
                    g_dLatentsIn, [1, Gs.components.synthesis.input_shape[1], Gs.input_shape[1]])
...
def loadProjector():
    ...
    g_Projector.regularize_noise_weight = float(
        os.environ.get('REGULARIZE_NOISE_WEIGHT', 1e5))
    g_Projector.initial_noise_factor = float(
        os.environ.get('INITIAL_NOISE_FACTOR', 0.05))
    g_Projector.uniform_latents = int(os.environ.get('UNIFORM_LATENTS', 0)) > 0
    g_Projector.euclidean_dist_weight = float(
        os.environ.get('EUCLIDEAN_DIST_WEIGHT', 1))
    g_Projector.regularize_magnitude_weight = float(
        os.environ.get('REGULARIZE_MAGNITUDE_WEIGHT', 0))
    ...
    with g_Session.as_default():
        if fromW != 0:
            ...
            if latents.shape[0] < g_dLatentsIn.shape[0]:
                latents = np.tile(
                    latents, g_dLatentsIn.shape[0] // latents.shape[0])
            images = dnnlib.tflib.run(synthesis, {g_dLatentsIn: latents})
            image = misc.convert_to_pil_image(
            ...
            images = gs.run(latents, None, truncation_psi=psi,
                            randomize_noise=randomize_noise != 0, output_transform=fmt)
...
def main(argv):
    ...
    try:
        app.run(port=int(os.getenv('HTTP_PORT')),
                host = '0.0.0.0', threaded=False)
    except:
